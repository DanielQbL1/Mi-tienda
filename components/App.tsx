...
const AppContent: React.FC = () => {
  const { 
      ...
      toastMessage, showToast, isLoading
  } = useStore();
  ...
  const handleLoginSuccess = (u: any) => {
      // User is already set in StoreContext by loginUser/registerUser
      setIsAuthOpen(false);
      
      // Check for admin role to redirect. 
      // u.role might be 'user' (dummy object from AuthView), but user context handles admin update.
      // We check u.email as well to be safer if AuthView passed dummy role.
  ...
  if (isLoading) {
    return (
        <div className="min-h-screen bg-white flex flex-col items-center justify-center animate-fade-in">
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6 animate-pulse" style={{ backgroundColor: settings.primaryColor }}>
                <span className="text-white font-black text-2xl">{settings.name ? settings.name.charAt(0) : 'S'}</span>
            </div>
            <div className="w-8 h-8 border-4 border-gray-100 border-t-black rounded-full animate-spin" style={{ borderTopColor: settings.primaryColor }} />
        </div>
    );
  }
